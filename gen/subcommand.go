...
// command name (from Command attr or lowercase variant name)
// description
// index in the enum
// the command struct type (e.g., GreetCmd)
// the enum wrapper struct type (e.g., Cmd_Greet)
...
func ParseSub[P any, S any]() (P, S, error) {
	return ParseArgsSub[P, S](os.Args[1:])
}
...
func ParseArgsSub[P any, S any](args []string) (P, S, error) {
	...
	for i, arg := range args {
		...
		if parentMeta.version != "" && (arg == "-V" || arg == "--version") {
			return parent, sub, ParseError{
				Kind: ErrorKindVersionRequested,
	...
	_err0 := parseInto(parentVal, (&globalMeta), globalArgs)
	if _err0 != nil {
		err := _err0
	...
	_err1 := parseInto(subCmdVal, (&subCmdMeta), subArgs)
	if _err1 != nil {
		err := _err1
		return parent, sub, err
	}
